...
def main():
    ...
    if upStatus == -1:
        ...
        gpu_fan, gpu_temp, gpu_power_usage, gpu_power_cap, gpu_mem_usage, gpu_mem_cap, gpu_util = \
            logs[0], logs[1], logs[3], logs[5], logs[7], logs[9], logs[11][:-1]

        template_image_icon = PCIE_GPU_ICON
        print('|templateImage={}'.format(PCIE_GPU_ICON))
        print('---')
        print('Connected to {}'.format(instance_name))
        print('---')
        print(name)
        print('Fan: {}'.format(gpu_fan))
        print('Temp: {}'.format(gpu_temp))
        print('Power: {}/{}'.format(gpu_power_usage, gpu_power_cap))
        print('Memory: {}/{}'.format(gpu_mem_usage, gpu_mem_cap))
        print('Util: {}'.format(gpu_util))
    else:
        output = subprocess.check_output(["system_profiler", "-xml", "SPDisplaysDataType"])
        plist = plistlib.readPlistFromBytes(output)
